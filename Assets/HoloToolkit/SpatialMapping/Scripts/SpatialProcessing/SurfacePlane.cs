...
namespace HoloToolkit.Unity.SpatialMapping
{
	...
	[Tooltip("Emergency UI Earthquake Warning")]
        public GameObject earthquakeWarning;

        [Tooltip("Emergency UI Safe Zone")]
        ...
        private void SetPlaneType()
        {
            ...
            if (SurfaceNormal.y >= UpNormalThreshold)
            {
                ...
                if (gameObject.transform.position.y > (floorYPosition + FloorBuffer))
                {
                    ...
                    Debug.Log("creating safety warning");
                    menu = Instantiate(safeZone);
                    menu.transform.position = plane.Bounds.Center;
                    menu.transform.rotation = plane.Bounds.Rotation;
                }
            }
            ...
            else if (Mathf.Abs(SurfaceNormal.y) <= (1 - UpNormalThreshold))
            {
                ...
                Debug.Log("creating earthquake warning");
                menu = Instantiate(earthquakeWarning);
                menu.transform.position = plane.Bounds.Center;
                menu.transform.rotation = plane.Bounds.Rotation;
            }
            else
